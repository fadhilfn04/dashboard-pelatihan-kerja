  ...
  <div class="max-w-lg">
    ...
    >
      <ul class="h-48 space-y-1 overflow-y-auto p-3 text-sm text-gray-700">
        ...
        <li v-for="programPelatihan in programPelatihanList" :key="programPelatihan.id">
          <div class="flex items-center rounded p-2 hover:bg-gray-100">
            <input
...
<script>
export default {
  name: "FilterTingkatPenyerapanLulusProgramPelatihan",
  props: {
    tipe: {
      type: String,
      default: null,
  ...
  methods: {
    emitEvent(id) {
      ...
      this.$emit("programPelatihanTingkatPenyerapanLulusChanged", data);
    },
  },
};
</script>