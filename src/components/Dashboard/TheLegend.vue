<template>
  ...
  >
    Jumlah Keseluruhan Lembaga Pelatihan Kerja: {{ summaryTrainingInstitutions }}
  </div>
  ...
  methods: {
    ...
    getSummaryTrainingInstitutions() {
      ...
      axios
        ...
        .then((response) => {
          if (response.data) {
            this.summaryTrainingInstitutions = response.data.data.toLocaleString();
          }
        });
    },
  },
  mounted() {
    this.getSummaryTrainingInstitutions();
  },
  computed: {
    formattedLegends() {
