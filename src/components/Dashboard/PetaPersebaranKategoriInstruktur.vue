...
export default {
  ...
  data() {
    return {
      ...
      api: "/recap-instructor-category-percentage",
  ...
  props: {
    selectedYearSebaranInstruktur: {
      type: Number,
      default: null,
    },
  },
  watch: {
    selectedYearSebaranInstruktur(newYear) {
  ...
  methods: {
    updateApi(newYear) {
      ...
      if (newYear) {
        this.api = `/recap-instructor-category-percentage-year/${newYear}`;
      } else {
        this.api = `/recap-instructor-category-percentage-year/${currentYear}`;
      }
      this.fetchData();
    },
    fetchData() {
